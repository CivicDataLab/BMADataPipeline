...
def insert_data_to_db(table_name: str, data: List[Dict[str, Any]], db_type=None):
    """
...
def get_json_data(url: str, headers: Optional[Dict[str, str]] = None,
    ...
    """
    response = make_api_request(url=url, method='GET', headers=headers, params=params, auth=auth, cookies=cookies)
...
def post_data(url: str, data: Dict[str, Any], headers: Optional[Dict[str, str]] = None,
    ...
    """
    response = make_api_request(url=url, method='POST', headers=headers, data=data, auth=auth)
    return response.json()
...
def paginated_api_request(base_url: str, params: Dict[str, Any], headers: Optional[Dict[str, str]] = None,
    ...
    while True:
        ...
        response = make_api_request(url=base_url, method='GET', headers=headers, params=params)
        response_data = response.json()
        
        # Extract data based on data_key if provided
        page_data = response_data.get(data_key, response_data) if data_key else response_data
...
def get_bma_api_auth():
    ...
    if not username or not password:
        raise ValueError("BMA API credentials not found in environment variables")
...
def api_to_db_pipeline(api_url: str, table_name: str, headers: Optional[Dict[str, str]] = None,
    ...
    try:
        # Fetch data from API
        if is_paginated:
            data = paginated_api_request(api_url, params or {}, headers, data_key=data_key)
        else:
...
# API calls to the translation and named entity extraction service
